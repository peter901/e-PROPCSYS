<div class="dragable" id="input_datetime">Datetime Picker</div>
<div class="element_code" id="input_datetime_element">
	<label for="<?php echo $element_params['input_id']; ?>" id="input_datetime_{n}_label" class="datetime_label updatable_label"><?php echo $element_params['label_text']; ?></label>
...
<div class="element_config" id="input_datetime_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('general'); ?>
		...
		<?php echo $HtmlHelper->input('input_datetime_{n}_input_name_config', array('type' => 'text', 'label' => 'Field Name', 'class' => 'small_input', 'value' => '', 'smalldesc' => 'No spaces or any special characters, but underscores _ and square brackets [] are allowed.')); ?>
		...
		<?php echo $HtmlHelper->input('input_datetime_{n}_input_title_config', array('type' => 'text', 'label' => 'Field Title', 'class' => 'small_input', 'value' => '', 'smalldesc' => 'Change this to override the default JS validation error message.')); ?>
	...
	<?php echo $PluginTabsHelper->tabStart('validation'); ?>
		<span style="color:#ff0000;">You should have the 2 checkboxes below checked in order for the "Default" date/time picker validation to work correctly, but if you are using the "Mootools Picker" then only "Required" should be checked.</span><br />
		<?php echo $HtmlHelper->input('input_datetime_{n}_validations_config', array('type' => 'checkbox', 'label' => 'Required', 'class' => 'small_input', 'value' => 'required', 'rule' => "split", 'splitter' => ",")); ?>
		<?php echo $HtmlHelper->input('input_datetime_{n}_validations_config', array('type' => 'checkbox', 'label' => 'Point to Clone', 'class' => 'small_input', 'value' => 'target:%field_name%_clone_id', 'rule' => "split", 'splitter' => ",", "smalldesc" => 'You must enable this for the "Default" time picker ONLY!')); ?>
	<?php echo $PluginTabsHelper->tabEnd(); ?>
</div>