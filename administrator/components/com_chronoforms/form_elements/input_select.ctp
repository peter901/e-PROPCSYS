<div class="dragable" id="input_select">Drop Down</div>
<div class="element_code" id="input_select_element">
	<label id="input_select_{n}_label" class="text_label updatable_label"><?php echo $element_params['label_text']; ?></label>
		<?php
		if(strlen(trim($element_params['options'])) > 0){
		...
		?>
		<select name="select_{n}_input_name" id="select_{n}_input_id" class="options_refelctor">
	...
	<?php echo $HtmlHelper->input('chronofield[{n}][input_select_{n}_enable_dynamic_data]', array('type' => 'hidden', 'id' => 'input_select_{n}_enable_dynamic_data', 'value' => $element_params['enable_dynamic_data'])); ?>
	<?php echo $HtmlHelper->input('chronofield[{n}][input_select_{n}_data_path]', array('type' => 'hidden', 'id' => 'input_select_{n}_data_path', 'value' => $element_params['data_path'])); ?>
	<?php echo $HtmlHelper->input('chronofield[{n}][input_select_{n}_value_key]', array('type' => 'hidden', 'id' => 'input_select_{n}_value_key', 'value' => $element_params['value_key'])); ?>
	<?php echo $HtmlHelper->input('chronofield[{n}][input_select_{n}_text_key]', array('type' => 'hidden', 'id' => 'input_select_{n}_text_key', 'value' => $element_params['text_key'])); ?>
...
<div class="element_config" id="input_select_element_config">
	<?php echo $PluginTabsHelper->Header(array('general' => 'General', 'other' => 'Other', 'validation' => 'Validation', 'dynamic_data' => 'Dynamic Data'), 'input_select_element_config_{n}'); ?>
	<?php echo $PluginTabsHelper->tabStart('general'); ?>
		...
		<?php echo $HtmlHelper->input('input_select_{n}_input_name_config', array('type' => 'text', 'label' => 'Field Name', 'class' => 'small_input', 'value' => '', 'smalldesc' => 'No spaces or any special characters, but underscores _ and square brackets [] are allowed.')); ?>
		<?php echo $HtmlHelper->input('input_select_{n}_input_title_config', array('type' => 'text', 'label' => 'Field title', 'class' => 'small_input', 'value' => '', 'smalldesc' => 'Change this to override the default JS validation error message.')); ?>
	...
	<?php echo $PluginTabsHelper->tabStart('dynamic_data'); ?>
		<?php echo $HtmlHelper->input('input_select_{n}_enable_dynamic_data_config', array('type' => 'select', 'label' => 'Enable', 'options' => array(0 => 'No', 1 => 'Yes'), 'default' => '1', 'smalldesc' => 'Enable Dynamic options loading, the options should exist in the $form->data array.')); ?>
		<?php echo $HtmlHelper->input('input_select_{n}_data_path_config', array('type' => 'text', 'label' => 'Data Path', 'value' => '', 'smalldesc' => 'The data path in the $form->data array, can be a MODEL ID, or a path using dots: MODEL1.MODEL2')); ?>
		<?php echo $HtmlHelper->input('input_select_{n}_value_key_config', array('type' => 'text', 'label' => 'Value Key', 'smalldesc' => 'The key name under which each option value will be found, so for example, if you have a multi list of users data loaded, then you may enter here "id"')); ?>
		<?php echo $HtmlHelper->input('input_select_{n}_text_key_config', array('type' => 'text', 'label' => 'Text Key', 'smalldesc' => 'The key name under which each option text will be found, so for example, if you have a multi list of users data loaded, then you may enter here "username"')); ?>
