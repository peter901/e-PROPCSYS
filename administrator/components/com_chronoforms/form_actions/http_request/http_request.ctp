...
<div class="element_code" id="cfaction_http_request_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_http_request_{n}_onrequest_fn]" id="action_http_request_{n}_onrequest_fn" value="<?php echo $action_params['onrequest_fn']; ?>" />
	<input type="hidden" name="chronoaction[{n}][action_http_request_{n}_onsuccess_fn]" id="action_http_request_{n}_onsuccess_fn" value="<?php echo $action_params['onsuccess_fn']; ?>" />
	<input type="hidden" name="chronoaction[{n}][action_http_request_{n}_onfailure_fn]" id="action_http_request_{n}_onfailure_fn" value="<?php echo $action_params['onfailure_fn']; ?>" />
...
<div class="element_config" id="cfaction_http_request_element_config">
	<?php echo $PluginTabsHelper->Header(array('settings' => 'Settings', 'advanced' => 'Advanced', 'help' => 'Help'), 'http_request_config_{n}'); ?>
	...
	<?php echo $PluginTabsHelper->tabStart('advanced'); ?>
		<?php echo $HtmlHelper->input('action_http_request_{n}_onrequest_fn_config', array('type' => 'text', 'label' => "On Request Function name", 'class' => 'medium_input', 'smalldesc' => "A JS function name to run when the request is sent, please add the 2 brackets at the end, e.g: myfn()")); ?>
		<?php echo $HtmlHelper->input('action_http_request_{n}_onsuccess_fn_config', array('type' => 'text', 'label' => "On Success Function name", 'class' => 'medium_input', 'smalldesc' => "A JS function name to run when the request is completed successfully, please add the 2 brackets at the end, e.g: myfn()")); ?>
		<?php echo $HtmlHelper->input('action_http_request_{n}_onfailure_fn_config', array('type' => 'text', 'label' => "On Failure Function name", 'class' => 'medium_input', 'smalldesc' => "A JS function name to run when the request has failed, please add the 2 brackets at the end, e.g: myfn()")); ?>
	...
	<?php echo $PluginTabsHelper->tabStart('help'); ?>
		<p>
			<ul>
				...
				<li>For example, set your form's action URL to PayPal, Moneybookers or SugarCRM..etc and use this HTTP Request to communicate with one of your form events, or vice versa.</li>
				<li>If you want to disable the form submission, add this code to the "Form tag attachment" field: onSubmit="return false;"</li>
			</ul>
		</p>
	<?php echo $PluginTabsHelper->tabEnd(); ?>
</div>