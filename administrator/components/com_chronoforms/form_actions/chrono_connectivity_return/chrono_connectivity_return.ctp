...
<div class="element_code" id="cfaction_chrono_connectivity_return_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_chrono_connectivity_return_{n}_connection_name]" id="action_chrono_connectivity_return_{n}_connection_name" value="<?php echo $action_params['connection_name']; ?>" />
...
<div class="element_config" id="cfaction_chrono_connectivity_return_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('settings'); ?>
		...
		<?php echo $HtmlHelper->input('action_chrono_connectivity_return_{n}_connection_name_config', array('type' => 'text', 'label' => "Connection Name", 'class' => 'medium_input', 'smalldesc' => 'Leave empty for auto integration, if no connection name is passed for this action then this will be used.')); ?>
