...
<div class="element_code" id="cfaction_file_downloader_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_file_downloader_{n}_display]" id="action_file_downloader_{n}_display" value="<?php echo $action_params['display']; ?>" />
	...
	<input type="hidden" name="chronoaction[{n}][action_file_downloader_{n}_file_name_path]" id="action_file_downloader_{n}_file_name_path" value="<?php echo $action_params['file_name_path']; ?>" />
...
<div class="element_config" id="cfaction_file_downloader_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('settings'); ?>
		<?php echo $HtmlHelper->input('action_file_downloader_{n}_display_config', array('type' => 'select', 'label' => "Download Type", 'options' => array('D' => 'Download', 'I' => 'Inline'), 'smalldesc' => "Select what the user is going to get.")); ?>
		<?php echo $HtmlHelper->input('action_file_downloader_{n}_downloads_path_config', array('type' => 'text', 'label' => "Downloads Path", 'class' => 'big_input', 'smalldesc' => "The absolute path to the downloads directory on the server.<br />You can use \"JOOMLA_PATH\" in the path, it will be replaced by the website path in runtime.")); ?>
		...
		<?php echo $HtmlHelper->input('action_file_downloader_{n}_file_name_path_config', array('type' => 'text', 'label' => "File Name Path", 'class' => 'medium_input', 'smalldesc' => "The path to the file name in the data array, helpful when processing dynamic data.")); ?>
