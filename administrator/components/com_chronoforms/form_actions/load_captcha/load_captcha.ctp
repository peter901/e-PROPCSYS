...
<div class="element_code" id="cfaction_load_captcha_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_load_captcha_{n}_refresh_button]" id="action_load_captcha_{n}_refresh_button" value="<?php echo $action_params['refresh_button']; ?>" />
...
<div class="element_config" id="cfaction_load_captcha_element_config">
	...
	<?php echo $HtmlHelper->input('action_load_captcha_{n}_refresh_button_config', array('type' => 'select', 'label' => "Refresh Button", 'options' => array(0 => 'Disabled', 1 => 'Enabled'), 'smalldesc' => 'Add a refresh image button beside the image, this can NOT be enabled if the encoded image setting is enabled.')); ?>
