...
<div class="element_code" id="cfaction_show_html_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_show_html_{n}_form_container]" id="action_show_html_{n}_form_container" value="<?php echo $action_params['form_container']; ?>" />
...
window.addEvent('domready', function() {
	refreshEventsList_showHTML('{n}', 1);									
});
var counter = 0;
function refreshEventsList_showHTML(SID, newCall, container, prefix){
	...
	new Element('option', {'value':'_self', 'text': 'Self'}).inject($('action_show_html_'+SID+'_submit_event_config'));
...
<div class="element_config" id="cfaction_show_html_element_config">
	<?php echo $PluginTabsHelper->Header(array('general' => 'General', 'multi_page' => 'Advanced (Multi Page)'), 'cfaction_show_html_element_config_{n}'); ?>
	...
	<?php echo $PluginTabsHelper->tabStart('multi_page'); ?>
		<input type="button" name="action_show_html_submit_event_refresh_button" id="action_show_html_submit_event_refresh_button" value="Refresh Events List" onClick="refreshEventsList_showHTML('{n}', 1)" />
		<br />
		<br />
		<?php echo $HtmlHelper->input('action_show_html_{n}_submit_event_config', array('type' => 'select', 'label' => 'Submission Event', 'options' => array('_self' => 'Self', 'load' => 'load', 'submit' => 'submit'), 'smalldesc' => 'Choose the event which will be executed when the form is submitted.')); ?>
		...
		<?php echo $HtmlHelper->input('action_show_html_{n}_form_container_config', array('type' => 'text', 'label' => 'Div container id', 'smalldesc' => 'Enter an id and it will be assigned to a div element wrapping the form element in the page.')); ?>
