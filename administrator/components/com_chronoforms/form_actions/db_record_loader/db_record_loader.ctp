...
<div class="element_code" id="cfaction_db_record_loader_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_db_record_loader_{n}_params_fields]" id="action_db_record_loader_{n}_params_fields" value="<?php echo $action_params['params_fields']; ?>" />
...
<div class="element_config" id="cfaction_db_record_loader_element_config">
	<?php echo $PluginTabsHelper->Header(array('basic' => 'Basic', 'advanced' => 'Advanced', 'help' => 'Help'), 'db_record_loader_config_{n}'); ?>
		<?php echo $PluginTabsHelper->tabStart('basic'); ?>
			...
			<?php echo $HtmlHelper->input('action_db_record_loader_{n}_request_param_config', array('type' => 'text', 'label' => "Request Param", 'class' => 'medium_input', 'value' => '', 'smalldesc' => "The param name which will exist in the request url to the form, its value will be used to load the target db record, if the value of this parameter is an array then the array values will be used inside 'IN' statement.")); ?>
		...
		<?php echo $PluginTabsHelper->tabStart('advanced'); ?>
			...
			")); ?>
			<?php echo $HtmlHelper->input('action_db_record_loader_{n}_array_fields_sets_config', array('type' => 'text', 'label' => "Array Fields Sets", 'class' => 'medium_input', 'value' => '', 'smalldesc' => "list of fields of types array stored in the table, fields values will be extracted, you can use single or multiple sets, e.g:<br />field1,field2 OR field1,field2-field3,field4.")); ?>
			<?php echo $HtmlHelper->input('action_db_record_loader_{n}_array_separators_config', array('type' => 'text', 'label' => "Array Separators", 'class' => 'medium_input', 'value' => '', 'smalldesc' => "The separators used to explode the array fields values, multiple sets supported, should match the number of sets for the array fields.")); ?>
			<?php echo $HtmlHelper->input('action_db_record_loader_{n}_params_fields_config', array('type' => 'text', 'label' => "Parameters fields", 'class' => 'medium_input', 'value' => '', 'smalldesc' => "Comma separated list of fields which will be in Joomla params fields format, values will be extracted to an array.")); ?>
		<?php echo $PluginTabsHelper->tabEnd(); ?>
		<?php echo $PluginTabsHelper->tabStart('help'); ?>
			<p>
				<ul>
					<li>Select the table name to load the data from, only the first record from the results will be loaded.</li>
					<li>Your table must have a primary key.</li>
					<li>Please give your table a unique model id, e.g: jos_content can have "Article".</li>
					<li>You can order the results and so control which record will be returned, use the "ORDER BY" word in the WHERE box. e.g: ORDER BY `id`.</li>
				</ul>
			</p>
		<?php echo $PluginTabsHelper->tabEnd(); ?>
</div>