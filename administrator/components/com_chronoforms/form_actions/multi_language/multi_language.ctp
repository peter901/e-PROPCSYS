...
<div id="cfaction_multi_language_main_tooltip" style="display:none;">
	<font class="tooltip-header">Multi Language</font><br />
	<p class="tooltip_container">
		Add extra languages support to your form:
		<br />
		1- drag one for every language you need supported.<br />
		2- translates the data in the same Event its inside only.		
	</p>
</div>
<!--start_element_code-->
<div class="element_code" id="cfaction_multi_language_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_multi_language_{n}_translate_output]" id="action_multi_language_{n}_translate_output" value="<?php echo $action_params['translate_output']; ?>" />
...
<div class="element_config" id="cfaction_multi_language_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('settings'); ?>
		...
		<?php echo $HtmlHelper->input('action_multi_language_{n}_translate_output_config', array('type' => 'select', 'label' => "Translate output", 'options' => array(0 => 'No', 1 => 'Yes'), 'smalldesc' => "Should the form output be scanned for translatable strings ? this option should not be needed unless you have dynamic text generation in one of your form elements/actions.")); ?>
