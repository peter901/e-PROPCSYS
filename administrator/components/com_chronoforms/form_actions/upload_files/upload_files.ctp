...
<div class="element_code" id="cfaction_upload_files_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_upload_files_{n}_array_fields]" id="action_upload_files_{n}_array_fields" value="<?php echo $action_params['array_fields']; ?>" />
...
<div class="element_config" id="cfaction_upload_files_element_config">
	<?php echo $PluginTabsHelper->Header(array('settings' => 'Settings', 'advanced' => 'Advanced', 'help' => 'Help'), 'upload_files_config_{n}'); ?>
	<?php echo $PluginTabsHelper->tabStart('settings'); ?>
		...
		<?php echo $HtmlHelper->input('action_upload_files_{n}_files_config', array('type' => 'text', 'label' => "Fields Configuration", 'class' => 'big_input', 'value' => '', 'smalldesc' => 'Config string, e.g: field1:jpg-png-gif,field2:zip-rar,field3:doc-docx-pdf')); ?>
		<?php echo $HtmlHelper->input('action_upload_files_{n}_upload_path_config', array('type' => 'text', 'label' => "Upload Path", 'class' => 'big_input', 'smalldesc' => 'The files upload path, if left empty, files will be uploaded under this path:<br />JOOMLA_PATH/components/com_chronoforms/uploads/FORM_NAME/<br />You can use "JOOMLA_PATH" in the path, it will be replaced by the website path in runtime.')); ?>
	...
	<?php echo $PluginTabsHelper->tabStart('advanced'); ?>
		<?php echo $HtmlHelper->input('action_upload_files_{n}_array_fields_config', array('type' => 'text', 'label' => "Array Fields", 'class' => 'medium_input', 'value' => '', 'smalldesc' => 'Fields names of type array []')); ?>
	<?php echo $PluginTabsHelper->tabEnd(); ?>
	<?php echo $PluginTabsHelper->tabStart('help'); ?>
		<p>
			<ul>
				...
				<li>Files data will be stored after processing under the $form->files AND $form->data['_PLUGINS_']['upload_files'].</li>
