...
<div class="element_code" id="cfaction_email_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_email_{n}_replace_nulls]" id="action_email_{n}_replace_nulls" value="<?php echo $action_params['replace_nulls']; ?>" />
...
<div class="element_config" id="cfaction_email_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('template'); ?>
		...
		<?php echo $HtmlHelper->input('action_email_{n}_replace_nulls_config', array('type' => 'select', 'label' => "Replace Nulls", 'options' => array(0 => 'No', 1 => 'Yes'), 'smalldesc' => 'Should all the curly brackets strings be replaced ? even if there is no key for this field in the data array ?')); ?>
