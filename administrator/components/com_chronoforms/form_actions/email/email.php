...
class CfactionEmail{
	var $formname;
	var $formid;
	var $details = array('title' => 'Email', 'tooltip' => 'Send an E-mail message.');
	...
	function run($form, $actiondata){
		...
		if(strlen(trim($email_params->get("attachments", ""))) && !empty($form->files)){
			...
			foreach($attachments as $attachment){
				if(isset($form->files[$attachment]['path'])){
					...
					if($form->files[$attachment] === array_values($form->files[$attachment])){
						foreach($form->files[$attachment] as $m_file){
							if(isset($m_file['path'])){
								$email_attachments[] = $m_file['path'];
							}
						}
					}
				}
	...
	function generate_table_list($elements_code = ''){
		$output = '<table cellpadding="5" cellspacing="5" border="0">';
		$output .= "\n";
