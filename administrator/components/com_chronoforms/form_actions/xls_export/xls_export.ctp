...
<div class="element_code" id="cfaction_xls_export_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_xls_export_{n}_before_headers]" id="action_xls_export_{n}_before_headers" value="<?php echo $action_params['before_headers']; ?>" />
...
<div class="element_config" id="cfaction_xls_export_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('settings'); ?>
		...
		<?php echo $HtmlHelper->input('action_xls_export_{n}_before_headers_config', array('type' => 'text', 'label' => 'Before Headers', 'class' => 'medium_input', 'smalldesc' => 'param name containing some data to be included before the headers, as a main title for document, the param should be set in the form data array.')); ?>
