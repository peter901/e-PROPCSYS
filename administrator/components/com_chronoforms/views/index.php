...
<span class="cf_alert"><div class="chronoform"><a href="http://www.chronoengine.com">Powered By ChronoForms - ChronoEngine.com</a></div></span>
<?php endif; ?>
...
<?php $active_app = $mainframe->getUserStateFromRequest('com_chronoforms.app', 'select_app', '', 'string'); ?>
<div id="forms_app" style="padding:5px 0px 5px 5px; width: 99%;">
	...
	<select name="select_app" id="select_app" onChange="$('adminForm').submit();">
		...
		<option value="<?php echo $k; ?>"<?php if($k == $active_app): ?> selected="selected"<?php endif; ?>><?php echo $app; ?></option>
		<?php endforeach; ?>
	</select>
</div>

<table class="gcore_table_list">
<thead>
	...
	<?php foreach($forms as $form): ?>
		...
		<tr>
			...
			<td width="1%" class='title'>
				<input type="checkbox" title="Checkbox for row 1" onclick="Joomla.isChecked(this.checked);" value="<?php echo $form->id; ?>" name="cid[]" id="cb<?php echo $i;?>">
			</td>
			<td width="10%" align="left" class='title'><a href="#edit" onclick="return listItemTask('cb<?php echo $i;?>','edit')"><?php echo $form->name; ?></a></td>
			<td width="10%" align="left" class='title'><a href="<?php echo JURI::Base().'index.php?option=com_chronoforms&task=form_wizard&form_id='.$form->id.'&formname='.$form->name; ?>">Wizard edit</a></td>
